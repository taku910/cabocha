...
//
//  $Id: mmap.h 41 2008-01-20 09:31:34Z taku-ku $;
//
...
namespace CaboCha {
  ...
  public:
    ...
    bool open(const char *filename, const char *mode = "r") {
      ...
      CHECK_CLOSE_FALSE(hFile != INVALID_HANDLE_VALUE) << "CreateFile() failed: " << filename;
    ...
    bool open(const char *filename, const char *mode = "r") {
      ...
      if (std::strcmp(mode, "r") == 0)  flag = O_RDONLY;
      else if (std::strcmp(mode, "r+") == 0) flag = O_RDWR;
      else CHECK_CLOSE_FALSE(false) << "unknown open mode: " << filename;
      ...
      char *p;
      CHECK_CLOSE_FALSE((p = (char *)mmap(0, length, prot, MAP_SHARED, fd, 0)) != MAP_FAILED)
        << "mmap() failed: " << filename;

      text = reinterpret_cast<T *> (p);
#else
      text = new T [length];
      CHECK_CLOSE_FALSE(read(fd, text, length) >= 0) << "read() failed: " << filename;
#endif
      close__(fd);
      fd = -1;
